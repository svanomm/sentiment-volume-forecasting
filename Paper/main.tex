...
\author{Steven VanOmmeren\thanks{\href{mailto:sevanommeren@gmail.com}{sevanommeren@gmail.com}. We thank Arvind Sharma for providing helpful feedback throughout the development of this paper. A complete replication package of our analysis is available at \url{https://github.com/svanomm/sentiment-volume-forecasting}. Analysis datasets are available upon request.}}
\date{\today}
\maketitle
\begin{abstract}
\noindent
    When institutional traders buy or sell large amounts of stock, they often have to consider the impact of their trades on the market price. Existing literature has shown that trading volume can be reliably forecasted and can translate to reduced price impact on large trades. In this paper, we attempt to model 15-minute ahead trading volume for 7 U.S. airline stocks with a large set of financial variables as well as sentiment measures from the Global Database of Events, Language, and Tone (GDELT). Through a combination of feature engineering, model selection, and hyperparameter tuning, we achieve an out-of-sample $R^2$ of just over 70\% in the prediction of 15-minute-ahead trading volume, over a testing period of approximately 18 months. Our best model explains 15 percentage points more variation than a naive forecast, and is comparable with state-of-the-art models in the literature. We find that sentiment measures improve the model's predictions, but only marginally so. These results suggest that sentiment measures are not necessary for accurate trading volume prediction, but they do not hurt the model's performance either. 
\end{abstract}
\newpage
\tableofcontents
\newpage
...
\begin{equation}
    R^2_{OOS} = 1 - \frac{\sum_{i\in O} (y_i - \hat{y}_i)^2}{\sum_{i\in O} (y_i - \bar{y})^2}
\end{equation}
In the formula, $O$ denotes the set of testing data, i.e. data that were not used in forming $\hat{y}$, and $\bar{y}$ is the average value in $O$. One important aspect of the $R^2_{OOS}$ is that it is possible for models to have a negative $R^2_{OOS}$, if they perform worse than guessing the (out-of-sample) mean of the dependent variable.\footnote{While in-sample $R^2$ can also be negative in general, it is typically assumed to be non-negative. OLS models with a constant are guaranteed to have a non-negative in-sample $R^2$. Note that mechanically, obtaining a positive $R^2_{OOS}$ is non-trivial because $\bar{y}$ is not known at the time of estimation.}

The following table reports the $R^2_{OOS}$ for each model and feature set. The results show that the LightGBM model with all features provides the best out-of-sample performance, with an $R^2_{OOS}$ of 0.7007. This is a slight improvement over the un-tuned model,\footnote{Hyperparameter tuning appeared to be unnecessary here, as it resulted in only a marginal improvement in performance.} and a non-trivial improvement over the OLS, LASSO, and Neural Network models. The LightGBM specification outperforms all other models for each feature set, reflecting its adept ability at recovering non-linear relationships in the data without overfitting.
...
The Sentiment Only model results show that OLS and the Neural Network models are unable to effectively utilize sentiment information, with the Neural Network model having a negative $R^2_{OOS}$. The LASSO and LightGBM models have low but non-trivial out-of-sample performance, with the LightGBM model clearly performing best at over double the $R^2_{OOS}$ of the LASSO model. This suggests that the sentiment measures provide a small but non-trivial amount of information for predicting stock volume when harnessed correctly.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.6\linewidth]{../Output/shap_abs_sentiment.pdf}
    \begin{minipage}{0.6\linewidth}
        \footnotesize
        \singlespacing
        Note: Each bar reports how much a given feature contributes (in absolute value) to the best model's predicted volume. The Shapley values are calculated over the entire testing period of 2023-12-04 to 2025-05-30.
...
\end{figure}
Besides the Care/Harm moral sentiment feature, the remaining features contribute very little to the model's predictions on average. Figure \ref{fig:shapley_sentiment_gdelt_top10} below shows the top 10 GDELT sentiment features, sorted by mean absolute Shapley value.\footnote{In Appendix Figure \ref{fig:shapley_sentiment_top20}, we report the overall top 20 sentiment features, including the LLM dimensions. The LLM dimensions often have higher mean absolute Shapley values than the GDELT features, but lack interpretability. Future work is needed to explore methods to better understand why certain LLM dimensions explain trading volume better than traditional sentiment features.}

\begin{figure}[H]
    \centering
    \caption{Top 10 GDELT Sentiment Features by Mean Absolute Shapley Value}
    \includegraphics[width=0.6\linewidth]{../Output/shap_abs_sentiment_gdelt_top10.pdf}
    \begin{minipage}{0.6\linewidth}
        \footnotesize
        \singlespacing
        Note: Each bar reports how much a given feature contributes (in absolute value) to the best model's predicted volume. The Shapley values are calculated over the entire testing period of 2023-12-04 to 2025-05-30.
    \end{minipage}
    \label{fig:shapley_sentiment_gdelt_top10}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\linewidth]{../Output/vwap_compare.pdf}
    \begin{minipage}{0.7\linewidth}
        \footnotesize
        \singlespacing
...
A table showing the MAPE for each model tested is shown below. The baseline model has a MAPE of 0.678. Lower MAPE represents a better model fit.
...
\subsection{Sentiment Shapley Values}

Below, we report the top 20 sentiment features by mean absolute Shapley value. This includes both GDELT sentiment features and the LLM dimensions. The LLM dimensions come from encoding the article's headline text into a 32-dimensional vector, which enters into the prediction model as 32 separate features. The LLM dimensions are often more important than the GDELT sentiment features, but they lack interpretability.
\begin{figure}[H]
    \centering
